...
≤a-weaken (≤a-hint x B≤H) nf = ≤a-hint (⊢a-weaken x (λ ()) {!!}) (≤a-weaken B≤H {!!})
...
... | ≤a-hint x A≤H = A≤H
⊢a-to-≤a (⊢a-ann ⊢a x) = x
⊢a-to-≤a (⊢a-lam₁ ⊢a) = ≤a-arr ≤a-refl-h {!!}
⊢a-to-≤a (⊢a-lam₂ ⊢a ⊢a₁ nf) = ≤a-hint (rebase ⊢a ≤a-refl-h) {!!}
  where
    rebase : ∀ {Γ e A B}
      → Γ ⊢a τ Top ⇛ e ⇛ B
      → Γ ⊢a B ≤ τ A
...
rebase-gen (⊢a-lam₂ ⊢f ⊢a nf) (have spl) ⊢e (ch-cons ch) = ⊢a-lam₂ ⊢f (rebase-gen ⊢a {!!} {!!} {!!}) {!!}
