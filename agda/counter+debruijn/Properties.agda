...
sound (⊢a-lam₂ ⊢e ⊢e₁) (have spl) = ⟨ (λ T≡Top → {!(proj₁ (sound ⊢e₁ spl)) T≡Top!}) , {!!} ⟩
...
complete (⊢d-app₂ ⊢e ⊢e₁) = {!!}
complete (⊢d-ann ⊢e) = {!!}
complete (⊢d-sub ⊢e x) = ⟨ (λ _ → {!(proj₂ (complete ⊢e)) refl!}) , (λ ()) ⟩

-- try induction on sub
sub-sub : ∀ {Γ A H e}
  → Γ ⊢a A ≤ H
  → Γ ⊢a τ Top ⇛ e ⇛ A
  → Γ ⊢a H ⇛ e ⇛ A
sub-sub ≤a-int ⊢e = {!!}
sub-sub ≤a-top ⊢e = {!!}
sub-sub (≤a-arr A≤H A≤H₁) ⊢e = {!!}
sub-sub (≤a-hint x A≤H) ⊢e = {!sub-sub A≤H ?!}
