...
rw-map : ∀ {Γ e xs x A j}
  → Γ ⊢ j # e ▻ (map (_↑ 0) xs ++ ⟦ x ↑ 0 ⟧) ⦂ A
  → Γ ⊢ j # e ▻ map (_↑ 0) (xs ++ ⟦ x ⟧) ⦂ A
...
subst' : ∀ (k) {Γ A B e e₁ j es}
  ...
  → Γ ⊢ Z # e₁ ⦂ A
  → Γ ⊢ j # ((ƛ e) · e₁) ▻ es ⦂ B
subst' (suc k) {es = []} sz ⊢1 ⊢2 = ⊢app₂ (⊢lam-n ⊢1) ⊢2
...
sound-≈ ≈□ none-□ = ⊩none
sound-≈ ≈τ none-τ = ⊩none
sound-≈ (≈term ⊢e A≈H) (have spl) = ⊩cons (sound-≈ A≈H spl) (sound-chk ⊢e none-τ)
...
sound-inf (⊢sub ⊢e A≈H p H≢□) spl = ⊢sub' (⊩-elim (sound-inf ⊢e none-□) (sound-≈ A≈H spl) spl)
...
sound-chk ty@(⊢sub ⊢e A≈H p H≢□) spl rewrite ⊢spl-eq ty spl = ⊢sub' (⊩-elim (sound-inf ⊢e none-□) (sound-≈ A≈H spl) spl)
