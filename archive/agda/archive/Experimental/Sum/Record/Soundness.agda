...
case-inside-0 : ∀ {Γ e e₁ e₂ es A₁ A₂ B i}
  → ¬ 1 ≤ size-apps es
  → Γ ⊢d ♭ Z # e ⦂ (A₁ ⊕ A₂)
  → Γ , A₁ ⊢d i # e₁ ▻ up 0 es ⦂ B
  → Γ , A₂ ⊢d i # e₂ ▻ up 0 es ⦂ B
  → Γ ⊢d i # case e [inj₁⇒ e₁ |inj₂⇒ e₂ ] ▻ es ⦂ B
case-inside-0 {es = es} sz ⊢e ⊢e₁ ⊢e₂ rewrite ¬<0→nil {es = es} sz = ⊢d-sum ⊢e ⊢e₁ ⊢e₂  

case-inside-3 : ∀ k₁ k₂ k₃ es {Γ e e₁ e₂ A₁ A₂ B i}
  → size-apps es < k₁
  → size-counter i < k₂
  → size-type B < k₃
  → Γ ⊢d ♭ Z # e ⦂ (A₁ ⊕ A₂)
...
case-inside-3-app : ∀ k₁ k₂ k₃ xs x {Γ e e₁ e₂ A₁ A₂ B i}
  → (1 + size-apps xs) < k₁
  → size-counter i < k₂
  → size-type B < k₃
  → Γ ⊢d ♭ Z # e ⦂ (A₁ ⊕ A₂)
  → Γ , A₁ ⊢d i # (e₁ ▻ (xs ⇈)) · (x ↑ 0) ⦂ B
  → Γ , A₂ ⊢d i # (e₂ ▻ (xs ⇈)) · (x ↑ 0) ⦂ B
  → Γ ⊢d i # (case e [inj₁⇒ e₁ |inj₂⇒ e₂ ] ▻ xs) · x ⦂ B
case-inside-3-app k₁ k₂ k₃ xs x sz₁ sz₂ sz₃ ⊢e (⊢d-app⇐ ⊢e₁ ⊢e₃) (⊢d-app⇐ ⊢e₂ ⊢e₄) =
  ⊢d-app⇐ {! !} (⊢d-strengthen-0 ⊢e₃) -- can be proved since two premises arrive in one application rule
case-inside-3-app k₁ k₂ k₃ xs x sz₁ sz₂ sz₃ ⊢e (⊢d-app⇐ ⊢e₁ ⊢e₃) (⊢d-app⇒ ⊢e₂ ⊢e₄) = {!!}
case-inside-3-app k₁ k₂ k₃ xs x sz₁ sz₂ sz₃ ⊢e (⊢d-app⇐ ⊢e₁ ⊢e₃) (⊢d-sub ⊢e₂ x₁ x₂) = {!!}
case-inside-3-app k₁ k₂ k₃ xs x sz₁ sz₂ sz₃ ⊢e (⊢d-app⇐ ⊢e₁ ⊢e₃) (⊢d-& ⊢e₂ ⊢e₄) = {!!}
case-inside-3-app k₁ k₂ k₃ xs x sz₁ sz₂ sz₃ ⊢e (⊢d-app⇒ ⊢e₁ ⊢e₃) ⊢e₂ = {!!}
case-inside-3-app k₁ k₂ k₃ xs x sz₁ sz₂ sz₃ ⊢e (⊢d-sub ⊢e₁ x₁ x₂) ⊢e₂ = {!!}
case-inside-3-app k₁ k₂ k₃ xs x sz₁ sz₂ sz₃ ⊢e (⊢d-& ⊢e₁ ⊢e₃) ⊢e₂ = {!!}

case-inside-3 (suc k₁) (suc k₂) (suc k₃) es (s≤s sz₁) (s≤s sz₂) (s≤s sz₃) ⊢e ⊢e₁ ⊢e₂ with size-apps es >? 0
case-inside-3 (suc k₁) (suc k₂) (suc k₃) es {e = e} {e₁ = e₁} {e₂ = e₂} (s≤s sz₁) (s≤s sz₂) (s≤s sz₃) ⊢e ⊢e₁ ⊢e₂ | yes p with apps-destruct es p
... | des-app x xs eq rewrite eq
                            | rw-apps-a xs (case e [inj₁⇒ e₁ |inj₂⇒ e₂ ]) x
                            | up-+++-distri-a xs x
                            | rw-apps-a (up 0 xs) e₁ (x ↑ 0)
                            | rw-apps-a (up 0 xs) e₂ (x ↑ 0)
    = {!!}
... | des-prj l xs eq = {!!}
case-inside-3 (suc k₁) (suc k₂) (suc k₃) es (s≤s sz₁) (s≤s sz₂) (s≤s sz₃) ⊢e ⊢e₁ ⊢e₂ | no ¬p = case-inside-0 {es = es} ¬p ⊢e ⊢e₁ ⊢e₂


case-inside : ∀ {Γ e e₁ e₂ es A₁ A₂ B i}
  → Γ ⊢d ♭ Z # e ⦂ (A₁ ⊕ A₂)
  → Γ , A₁ ⊢d i # e₁ ▻ up 0 es ⦂ B
  → Γ , A₂ ⊢d i # e₂ ▻ up 0 es ⦂ B
  → Γ ⊢d i # case e [inj₁⇒ e₁ |inj₂⇒ e₂ ] ▻ es ⦂ B
...
sound-inf {es = es} (⊢a-sum ⊢e ⊢e₁ ⊢e₂) spl with sound-inf-0 ⊢e | sound-inf ⊢e₁ (spl-weaken spl) | sound-inf ⊢e₂ (spl-weaken spl)
... | ind-e | ind-e₁ | ind-e₂ = case-inside {es = es} ind-e ind-e₁ ind-e₂
...
sound-chk {es = es} (⊢a-sum ⊢e ⊢e₁ ⊢e₂) spl with sound-inf-0 ⊢e | sound-chk ⊢e₁ (spl-weaken spl) | sound-chk ⊢e₂ (spl-weaken spl)
... | ind-e | ind-e₁ | ind-e₂ = case-inside {es = es} ind-e ind-e₁ ind-e₂
...
sound-≤-chk (≤a-sum x x₁) none-τ = ≤d-sum (sound-≤-chk x none-τ) (sound-≤-chk x₁ none-τ)
...
sound-es (≤a-sum x x₁) none-τ = ⊩none⇚
