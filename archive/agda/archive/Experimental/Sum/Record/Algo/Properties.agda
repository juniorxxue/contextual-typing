...
≤a-weaken (≤a-sum x x₁) = ≤a-sum (≤a-weaken x) (≤a-weaken x₁)
...
⊢a-weaken {H = H} {A = A} {n = n} {n≤l = n≤l} (⊢a-sum ⊢e ⊢e₁ ⊢e₂) with ⊢a-weaken {A = A} {n = suc n} {n≤l = s≤s n≤l} ⊢e₁ | ⊢a-weaken {A = A} {n = suc n} {n≤l = s≤s n≤l} ⊢e₂
... | ind-e₁ | ind-e₂ rewrite sym (⇧-⇧-comm-0 H n) = ⊢a-sum (⊢a-weaken ⊢e) ind-e₁ ind-e₂
...
⊢a-id (⊢a-sum ⊢e ⊢e₁ ⊢e₂) spl = ⊢a-id ⊢e₁ (spl-weaken spl)
...
≤a-strengthen (≤a-sum x₁ x₂) x n≤l = ≤a-sum (≤a-strengthen x₁ sdh-τ n≤l) (≤a-strengthen x₂ sdh-τ n≤l)
...
⊢a-strengthen {H = H} {n = n} (⊢a-sum ⊢e ⊢e₁ ⊢e₂) (sd-case sd sd₁ sd₂) sdh n≤l with ⊢a-strengthen ⊢e₁ sd₁ (⇧-shiftedh-n z≤n sdh) (s≤s n≤l)
                                                                                  | ⊢a-strengthen ⊢e₂ sd₂ (⇧-shiftedh-n z≤n sdh) (s≤s n≤l)
... | ind-e₁ | ind-e₂ rewrite sym (⇩-⇧-comm H 0 n z≤n sdh) = ⊢a-sum (⊢a-strengthen ⊢e sd sdh-□ n≤l) ind-e₁ ind-e₂
...
≤a-refined (≤a-sum x x₁) = ≤a-sum (≤a-refined x) (≤a-refined x₁)
...
... | ≤a-hint-l r = r
⊢a-to-≤a (⊢a-sum ⊢e ⊢e₁ ⊢e₂) = ≤a-strengthen-0 (⊢a-to-≤a ⊢e₁)
...
subsumption {H' = H'} (⊢a-sum ⊢e ⊢e₁ ⊢e₂) spl ch A≤H' | no ¬p = ⊢a-sum ⊢e (subsumption ⊢e₁ (spl-weaken spl) (ch-weaken ch) (≤a-weaken-0 A≤H'))
                                                                          (subsumption ⊢e₂ (spl-weaken spl) (ch-weaken ch) (≤a-weaken-0 A≤H'))
...
⊢a-spl-τ (⊢a-sum ⊢e ⊢e₁ ⊢e₂) spl = ⊢a-spl-τ ⊢e₁ (spl-weaken spl)
