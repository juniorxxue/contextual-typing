...
⊢d-weaken (⊢d-sum ⊢e ⊢e₁ ⊢e₂) n≤l = ⊢d-sum (⊢d-weaken ⊢e n≤l) (⊢d-weaken ⊢e₁ (s≤s n≤l)) (⊢d-weaken ⊢e₂ (s≤s n≤l))
...
⊢d-strengthen (⊢d-sum ⊢e ⊢e₁ ⊢e₂) (sd-case sd sd₁ sd₂) n≤l = ⊢d-sum (⊢d-strengthen ⊢e sd n≤l)
                                                                    (⊢d-strengthen ⊢e₁ sd₁ (s≤s n≤l))
                                                                    (⊢d-strengthen ⊢e₂ sd₂ (s≤s n≤l))
...
≤d-trans {B = B ⇒ B₁} (≤d-arr-S⇒ x₁ x₂) (≤d-arr-S⇒ x x₃) = ≤d-arr-S⇒ x (≤d-trans x₂ x₃)
...
≤d-trans {B = A ⊕ B} ≤d-Z B≤C = B≤C
≤d-trans {B = A ⊕ B} (≤d-sum A≤B A≤B₁) ≤d-top = ≤d-top
≤d-trans {B = A ⊕ B} (≤d-sum A≤B A≤B₁) (≤d-sum B≤C B≤C₁) = ≤d-sum (≤d-trans A≤B B≤C) (≤d-trans A≤B₁ B≤C₁)
≤d-trans {B = A ⊕ B} (≤d-sum A≤B A≤B₁) (≤d-and B≤C B≤C₁) = ≤d-and (≤d-trans (≤d-sum A≤B A≤B₁) B≤C)
                                                                  (≤d-trans (≤d-sum A≤B A≤B₁) B≤C₁)
≤d-trans {B = A ⊕ B} (≤d-and₁ A≤B x) B≤C = ≤d-and₁ (≤d-trans A≤B B≤C) x
≤d-trans {B = A ⊕ B} (≤d-and₂ A≤B x) B≤C = ≤d-and₂ (≤d-trans A≤B B≤C) x
